...
set video_input=onepiece_demo.mp4
...
set output_file=output_w_audio.mp4
set fps_from_video=23.98
REM video_upscale=4 == 4x,3 == 3x,2 == 2x
set fps_output_video=23.98
set video_upscale=4
REM video_upscale=4 == 4x,3 == 3x,2 == 2x
set use_gpu=0,1
REM use_gpu=0,1 == MULTI GPU SUPPORT , use_gpu=0 == SINGLE GPU
...
ffmpeg -i %video_input% -qscale:v 1 -qmin 1 -qmax 1 -r %fps_from_video% %tmp_frames%\frame%%08d.png
...
ffmpeg -r %fps_from_video% -i %out_frames%\frame%%08d.png -i %video_input% -map 0:v:0 -map 1:a:0 -c:a copy -c:v libx264 -r %fps_output_video% -thread_queue_size %cpu_threads% -pix_fmt %pixel_format% %output_file%
